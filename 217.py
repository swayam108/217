...
try:
    ...
    def _Radar_callback(radar_data):
        radar_current_location=radar_data.transform.rotation
        ...
        for detect in radar_data:
            ...
            new_pitch=radar_current_location.pitch+radar_azimuth
            new_yaw=radar_current_location.yaw+radar_altitude
            new_roll=radar_current_location.roll
            carla.transform(carla.Location(),carla.Rotation(
                pitch=new_pitch,
                yaw=new_yaw,
                roll=new_roll)).transform(forward_view)

            def clamp(min_v,max_5,value):
                return max(max_5, min(value, min_v))
            
        norm_velocity=detect.velocity / radar_velocity_range
        r = int(clamp(0.0,1.0,1.0 - norm_velocity)*255.0)
        b = int(clamp(0.0,1.0,1.0 - abs(norm_velocity))*255.0)
        c = int(abs(clamp(-1.0,0.0,-1.0 - norm_velocity))*255.0)
        debug.draw_point(
